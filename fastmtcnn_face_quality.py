...
sys.path.append(r'C:\Users\hutao\PycharmProjects\AI\check\applications\face\fastmtcnncollect')
import fastmtcnncollect_face_detect
...
def face_detect_single_pic(image):
    ...
    img_height = image.shape[0]
    # 人脸数rects
    face_boxes, face_datas=detector.detect_face(image)
    #print(rects)
    if face_boxes is not None:
        return face_boxes, img_wide, img_height
    else:
        return None, None, None
...
def face_vector(img):
    ...
    :return: 人脸图
    '''
    face_boxes, img_wide, img_height = face_detect_single_pic(img)
    if len(face_boxes) == 0:
        # print("没有检测到人脸")
        return None
    if face_boxes:
        for box in face_boxes:
            # 把人脸截取出来
            face_l_u_x, face_l_u_y, face_r_d_x, face_r_d_y = box[0], box[1], box[2], box[3]
            # 获取特征点(相对于原图片的)
            # print("正在提取特征点……")
...
class FaceQualityCheck():
    def __init__(self):
        ...
        self.clf_face = joblib.load(os.path.join(model_dir, 'models/3-6300.m'))
    ...
    def mouth_check(self, img_face_face, landmarks=None):
        ...
        try:
            ...
            # 有人脸但是特征点需要自己获取
                ...
                print("检测提取特征点耗时%.4fms" % ((time.time() - start_time_vectors) * 1000))
    ...
    # 质量判断方法
    def evaluate_quality(self, img_face, track_id=None, landmark=None):
        # 输入是人脸图片
        #start = time.time()
        ...
        #print('mouth_check: %.4fms' % ((time.time() - start) * 1000))
                ...
                if pred_face is not None:
                    if pred_face == 0:
                        # common_data.save_face_pic('angle_left', img, track_id + common_data.random_shortname())
                        return False, 0
                    elif pred_face == 1:
                        # common_data.save_face_pic('angle_right', img, track_id + common_data.random_shortname())
                        return False, 0
                    elif pred_face == 2:
                        # common_data.save_face_pic('post', img, track_id + common_data.random_shortname())
                        return True, float(angle_score)
                    # elif pred_face == 3:
                    #     # common_data.save_face_pic('no_face', img, track_id + common_data.random_shortname())
                    #     return False, 0
                else:
                    return False, 0
            #elif mouth_label == 1:  # 没嘴
                # common_data.save_face_pic('no_mouth', img, track_id + common_data.random_shortname())
                #return False, 0
        #else:
            #return False, 0
...
if __name__ == '__main__':
    ...
    detector = fastmtcnncollect_face_detect.face_detect(minsize=170)
    # dlib的68点模型
    ...
    while e:
        try:
            ...
            b = time.time()
            print('检测到人脸的时间%0.4fms' % ((b - a) * 1000))

            if img_face is not None:
                total_time_start = time.time()
                res, score = quality_check1.evaluate_quality(img_face, 1)
                over_time=time.time()
                print(score, '   total time %.4fms' % ((over_time - total_time_start) * 1000))

                c=c+((time.time() - total_time_start) * 1000)
                i=i+1
                agrv=c/i
                print(agrv,i)  #平均时间，次数

                if res is not None:
                    img_face = cv2.resize(img_face, (500, 500))
                    cv2.putText(img_face, str(res), (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
                    cv2.imshow('face_img', img_face)
                    cv2.waitKey(10)
        except Exception:
            pass
        finally:
