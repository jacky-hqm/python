...
def get_files(file_dir):
    ...
    image_list = np.hstack(( front,angle_left,angle_right))  # 所有图片列表
    label_list = np.hstack(( label_front,label_angle_left,label_angle_right))  # 所有标签列表
    # 利用shuffle打乱顺序
...
def tes(image_list, label_list):
    ...
    for i in range(0, len(image_list)):  # 测试集
        ...
        face_boxes, face_data=detector.detect_face(img1)

        #print(np.array(face_data).shape)
        #cv2.imshow('face',face_data[0])
        #cv2.waitKey(0)
        if face_data is None:
           res1 = cv2.resize(img1, (64, 64))
           no=no+1
           # print(image_list[i])
        else:
           res1 = cv2.resize(face_data[0], (64, 64))  # 对 v图片进行缩放，第一个参数是读入的图片，第二个是制定的缩放大小
...
def cal_time(image_list):
    # 载入模型
    clf1 = joblib.load(r"C:\Users\hutao\PycharmProjects\AI\check\applications\face\model_save\model.m")
    print('开始计算时间')
    total_time = 0.0
...
def predict(Test_image, Test_label, image_list, flag):
    # 载入模型
    clf = joblib.load(r"C:\Users\hutao\PycharmProjects\AI\check\applications\face\model_save\model.m")
    print("Predict test dataset ...")
    start = time.clock()
    y_pred = clf.predict(Test_image)

    # ay_pred = clf.predict_proba(Test_image)  # 对测试集的预测
    # b = np.max(ay_pred, axis=1)
    ...
    print(classification_report(Test_label, y_pred))
    print('预测完成')
    # return errfrontal,errleft,errright

def  move():
    path_img = r'C:\image\10.15\2018_10_12\1000\front'
    ls = os.listdir(path_img)
    for i in errleft:
        a = str(ls[i])
        shutil.move(path_img + '/' + a, test_dir+"\errleft" + '/' + a)
    for i in errright:
        a = str(ls[i])
        shutil.move(path_img + '/' + a, test_dir+"\errright" + '/' + a)
    for i in errfrontal:
        a = str(ls[i])
        shutil.move(path_img + '/' + a, test_dir+"\errfront" + '/' + a)


if __name__ == '__main__':
    # 设置文件路径
    # train_dir = r'E:\Samples\train_u\detect_face'
